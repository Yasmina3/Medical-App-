...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.configure(bg='#95F8E4')  # Set background color
        self.country_data = []
    ...
    def enter_num_countries(self):
        ...
        self.subtitle_label = tk.Label(self, text="Enter neighboring countries separated by commas i.e 1,2,3", font=("Helvetica", 14), bg='#95F8E4')
        self.subtitle_label.pack(pady=10)

        self.scrollable_frame = ttk.Frame(self, height=200)  
        self.scrollable_frame.pack(fill=tk.BOTH, expand=True)

        self.canvas = tk.Canvas(self.scrollable_frame, bg='#95F8E4', height=200)  
        ...
        self.inner_frame = tk.Frame(self.canvas, bg='#95F8E4')
        self.canvas.create_window((0, 0), window=self.inner_frame, anchor=tk.NW)

        self.country_entries = []
        for i in range(num_countries):
            label = tk.Label(self.inner_frame, text=f"Country {i}:", font=("Helvetica", 12, "bold"), bg='#95F8E4')
            label.pack()
