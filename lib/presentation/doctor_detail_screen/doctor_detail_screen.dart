...
class DoctorDetailScreen extends GetWidget<DoctorDetailController> {
  ...
  Widget build(BuildContext context) {
    ...
    String? doctorId = Get.arguments?["doctor_id"] as String?;
    int? doctorIdInt;
    ...
    print("In image doctor id = $doctorIdInt");
    ...
    return Scaffold(
      ...
      body: FutureBuilder<dynamic>(
        future: controller.fetchFromDetails(doctorIdInt!),
...
// Modify your _buildCalendar function
  ...
  dynamic getSessionId(var sessionList, DateTime sessionTime) {
    print("session list = ${sessionList}");
    ...
    for (var session in sessionList) {
      ...
      if (sessionDateTime.isAtSameMomentAs(sessionTime)) {
        ...
        return [session['session_id'],session["state"]];
      }
    }
          ...
          child: Wrap(
            ...
            children: List<Widget>.generate(
              controller.doctorDetailModelObj.value.Sessions_per_day.length,
                ...
                dynamic session_id_state = getSessionId(session_list,session_time);

                print("session is like with session_id=${session_id_state[0]} and state= ${session_id_state[1]}");
                model.am = Rx(time);
                model.session_id = Rx(session_id_state[0]);
                model.state = Rx(session_id_state[1]);
      ...
      onPressed: () {
        ...
        http.post(
          ...
          if (response.statusCode == 200) {
            ...
            Get.toNamed(AppRoutes.popupRequest);
          } else {
